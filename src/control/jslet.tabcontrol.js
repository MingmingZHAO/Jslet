 ...
 *		items: [
 *			{header: "one", userIFrame: true, url: "http://www.google.com", iconCls: "tabIcon"},
...
jslet.ui.TabControl = jslet.Class.create(jslet.ui.Control, {
    ...
    renderAll: function () {
        ...
        var w = parseInt(Z.el.style.width);
        if (!w)
            Z.el.style.width = '300px';

        var h = parseInt(Z.el.style.height);
        if (!h)
            Z.el.style.height = '300px';
        ...
        if (Z.newable) {
            ...
            newTab.onclick = function () {
                ...
                if (Z._rightIndex > 0)
                    Z._changeLeftIndex(1);//jslet.locale.isRtl ? -1 : 1)
            }
        }
    ...
    _createContextMenu: function () {
        ...
        if (Z.onCreateContextMenu)
            Z.onCreateContextMenu(menuCfg.items);
    ...
    _menuItemClick: function (menuid, checked, contextObj) {
        if (menuid == 'close')
            contextObj.close();
        else
            if (menuid == 'closeOther')
                contextObj.closeOther();
    },
    ...
    _refreshRightIndex: function () {
        ...
        var w = 0;
        var totalW = jQuery(oul.parentNode).width() - Z._naviBtnWidth;
        Z._rightIndex = 0;
        for (var i = cnt; i >= 0; i--) {
    ...
    _createHeader: function (parent, itemCfg) {
        var Z = this;
        var s = '<a href="javascript:;" class="jl-tab-inner" onclick="javascript:this.blur();"><span class="jl-tab-title ';
        var canClose = Z.closable && itemCfg.closable;
        if (canClose)
            s = s + 'jl-tab-close-loc ';

        if (itemCfg.iconCls)
            s = s + 'jl-tab-icon-loc ';
        s = s + '">' + itemCfg.header + '</span>';
        if (itemCfg.iconCls)
            s = s + '<span class="jl-tab-icon ' + itemCfg.iconCls + '"></span>';
        s = s + '</a>';
        if (canClose)
            s = s + '<a href="javascript:;" class="jl-tab-close"></a>';
    ...
    _doCloseBtnClick: function () {
        ...
        var nodes = oli.parentNode.childNodes;
        var index = -1;
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i] == oli) {
                index = i;
    ...
    _createBody: function (parent, itemCfg) {
        var Z = this;
        var jqDiv = jQuery(document.createElement('div'));
        if (!jqDiv.hasClass('jl-tab-panel'))
        	jqDiv.addClass('jl-tab-panel');
        var odiv = jqDiv[0];
        parent.appendChild(odiv);
        var padding = 4; //2*(odiv.style.padding?parseInt(odiv.style.padding):0);
        var jqEl = jQuery(Z.el);
        var jqHead = jqEl.find('.jl-tab-header');
        var w = jqEl.width() - padding;
        var h = jqEl.height() - padding - jqHead.height();
        if (w > 0)
        ...
        if (itemCfg.content || itemCfg.divId) {
            ...
            if (ocontent) {
                ...
                //                ocontent.style.width = w + 'px';
                //                ocontent.style.height = h + 'px';
                //                ocontent.style.paddingLeft = '5px';
                //                ocontent.style.paddingTop = '5px';
                return
            }
        }
        ...
        if (itemCfg.url) {
            if (itemCfg.useIFrame) {
                var s = '<iframe scrolling="yes" frameborder="0" src="' + itemCfg.url + '" style="padding-left:5px;padding-top:5px;width: ' + w + 'px;height:' + h + 'px;"></iframe>';
                jqDiv.html(s)
            }
        }
    },
     ...
     * 
     * @param {Integer} index Tab item index that will be toggle to.
     */
    setSelectedIndex: function (index) {
        var Z = this;
        var jqEl = jQuery(Z.el);
        var oli, oul = jqEl.find('.jl-tab-list')[0];
        var nodes = oul.childNodes;
        var cnt = nodes.length - (Z.newable ? 2 : 1);
        if (index > cnt || index < 0)
            return;

        if (Z.onSelectedChanged) {
            var canChanged = Z.onSelectedChanged(Z.selectedIndex, index);
            if (canChanged != undefined && !canChanged)
                return
        }

        var itemContainer = jqEl.find('.jl-tab-items')[0];
        var item, items = itemContainer.childNodes;
        for (var i = 0; i <= cnt; i++) {
            oli = jQuery(nodes[i]);
            item = items[i];
            if (i == index) {
    ...
    removeTabItem: function (tabIndex) {
        ...
        if (Z.onRemoveTabItem) {
            var canRemoved = Z.onRemoveTabItem(tabIndex, selected);
            if (!canRemoved)
                return
        }
...
jslet.ui.TabItem = function () {
    ...
    Z.iconCls; //{String} The icon class of tab item head 
