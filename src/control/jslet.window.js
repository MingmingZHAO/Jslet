 ...
 * Example:
 * <pre><code>
    var oWin = jslet.ui.createControl({ type: "Window", iconcls:"winicon", caption: "test window", minimizable: false, maximizable:false,onActive: doWinActive, onPositionChanged: doPositionChanged });
...
jslet.ui.Window = jslet.Class.create(jslet.ui.Control, {
    ...
    initialize: function ($super, el, params) {
        var Z = this;
        Z.el = el;
        Z.allProperties = 'caption,resizable,minimizable,maximizable,closable,iconcls,onSizeChanged,onClosed,onPositionChanged,onActive,width,height,minWidth,maxWidth,minHeight,maxHeight,isCenter';
        /**
    ...
    renderAll: function () {
        ...
        if (!Z.width)
            Z.width = parseInt(Z.el.style.width); // Z.el.getWidth();
        if (!Z.height)
            Z.height = parseInt(Z.el.style.height); // Z.el.getHeight();
        if (!Z.width)
            Z.width = 300;
        if (!Z.height)
            Z.height = 300;
        ...
        '<div class="jl-win-header jl-unselectable" style="position:relative;width: 100%;cursor:move">',
            Z.iconcls ? '<div class="jl-win-header-icon ' + Z.iconcls + '"></div>' : '',

            '<div class="jl-win-title jl-unselectable">', Z.caption ? Z.caption : '', '</div>',
            '<div class="jl-win-tool jl-unselectable">'];
        if(!jslet.locale.isRtl){
        	template.push(Z.minimizable ? '<a href="#" class="jl-win-min" onfocus="this.blur();"></a>' : '');
        	template.push(Z.maximizable ? '<a href="#" class="jl-win-max" onfocus="this.blur();"></a>' : '');
        	template.push(Z.closable ? '<a href="#" class="jl-win-close" onfocus="this.blur();"></a>' : '');
        }else{
        	template.push(Z.closable ? '<a href="#" class="jl-win-close" onfocus="this.blur();"></a>' : '');
        	template.push(Z.maximizable ? '<a href="#" class="jl-win-max" onfocus="this.blur();"></a>' : '');
        	template.push(Z.minimizable ? '<a href="#" class="jl-win-min" onfocus="this.blur();"></a>' : '');
...
//            event.preventDefault();
    ...
    show: function (left, top) {
        ...
        if (Z.isCenter) {
            ...
            var jqOffsetP = jQuery(offsetP);
            var pw = jqOffsetP.width();
            var ph = jqOffsetP.height();
    ...
    setPosition: function (left, top, notUpdateLeftTop) {
        ...
        if (!notUpdateLeftTop) {
            ...
            if (Z.onPositionChanged) {
                var result = Z.onPositionChanged(left, top);
                if (result) {
                    if (result.left)
                        left = result.left;
                    if (result.top)
...
//                window.status = Z.cursor;
    ...
    _doWinMouseMove: function (event) {
        ...
        if (x <= delta && y <= delta)
            ...
            if (x > delta && x < wdelta && y <= delta)
                cursor = 'n';
            else {
                if (x >= wdelta && y <= delta)
                    cursor = 'ne';
                else {
                    if (x >= wdelta && y > delta && y <= hdelta)
                        cursor = 'e';
                    else {
                        if (x >= wdelta && y >= hdelta)
                            cursor = 'se'
                        else {
                            if (x > delta && x < wdelta && y >= hdelta)
                                cursor = 's';
                            else {
                                if (x <= delta && y >= hdelta)
                                    cursor = 'sw'
                                else {
                                    if (x <= delta && y > delta && y < hdelta)
                                        cursor = 'w'
                                }
                            }
                        }
...
//            window.status = '';
//        window.status = window.status + ':' + cursor;
...
jslet.ui.MessageBox = function () {
	 ...
	 * @param {String} iconcls Caption icon class
	 ...
	 * 
	 */
    this.show = function (message, caption, iconcls, buttons, callbackFn, hasInput, defaultValue, validateFn) {
        ...
        var opt = { type: 'window', caption: caption, position: 'center', resizable: false, minimizable: false, maximizable: false, isCenter: true, height: mh, width: mw };
        var owin = jslet.ui.createControl(opt);
        var iconHtml = '';
        if (iconcls) {
            iconHtml = '<div class="jl-msg-icon ';
            if (iconcls == 'info' || iconcls == 'error' || iconcls == 'question' || iconcls == 'warning')
                iconHtml += 'jl-msg-icon-' + iconcls;
            else
                iconHtml += iconcls;
