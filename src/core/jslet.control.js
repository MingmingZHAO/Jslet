...
jslet.ui.Control = jslet.Class.create({
    ...
    initialize: function (el, params) {
        ...
        if (this.isValidTemplateTag	&& !this.isValidTemplateTag(this.el)) {
            var ctrlClass = jslet.getControlClass(params.type), template;
            if (ctrlClass)
...
jslet.ui.DBControl = jslet.Class.create(jslet.ui.Control, {
    ...
    renderInvalid: function (invalidMsg) {
        if (!this.field || !this.enableInvalidTip)
            return;
        var errFldObj = this.dataset.getErrorField(this.field);
        var invalidMsg = '';
        if (errFldObj){
            jQuery(this.el).addClass('jl-invalid');
            invalidMsg = errFldObj.invalidMsg;
        }
        else
        	jQuery(this.el).removeClass('jl-invalid');
        if(!jslet.ui.globalTip)
            this.el.title = invalidMsg || '';
        this.invalidMessage = invalidMsg;
    },
    ...
    checkDataField: function (fldName) {
        ...
        var f = this.dataset.getField(fldName);
        if (!f)
            throw new Error(jslet.formatString(jslet.locale.DBControl.fieldNotFound, [fldName]))
    },
...
jslet.ui.createControl = function (jsletparam, parent, width, height) {
    var isAuto = false, pnode = parent;
    var container = document.createElement('div');

    var p = jslet.ui._evalParams(jsletparam);
    var controlType = p.type;
    if (!controlType)
