...
jslet.data.Dataset = function (name) {
     ...
     *   //fieldValue: Object, field value, its datatype is same as field datatype.
     */
...
//        }
     ...
     */
    this.addErrorField = function(fldName, inputText, invalidMsg){
    	var Z = this;
    	if(!Z._errorFields)
    		Z._errorFields = {};
        Z._errorFields[fldName] = {inputText: inputText, invalidMsg: invalidMsg};
    }
     ...
     */
    this.getErrorField = function(fldName){
        if(!this._errorFields)
        	return null;
        return this._errorFields[fldName];       
    }
     ...
     * @param {String} fieldName Field name.
     */
    this.removeErrorField = function(fldName){
        if(!this._errorFields)
        	return;
        delete this._errorFields[fldName];       
    }
    ...
    this.getFieldText = function (fldName, inEdit) {
        ...
        var errFldObj = Z.getErrorField(fldName);
        if(errFldObj)
        ...
        if (fldObj.getType() == jslet.data.DataType.DATE) {
            ...
            return jslet.formatDate(value, fldObj.displayFormat())
        }
        return value
    }
    ...
    this.setFieldText = function (fldName, inputText, keyValue, displayValue) {
        ...
        var invalidMsg = fldObj.validator.checkInputText(inputText);
        if(invalidMsg){
        	Z.addErrorField(fldName, inputText, invalidMsg);
        	return;
        }else{
        	Z.removeErrorField(fldName);
        }
        ...
        if (lkFld && fldObj.clientTranslate()) {
            ...
            if(value === null){
            	invalidMsg = jslet.formatString(jslet.locale.Dataset.valueNotFound);
            	Z.addErrorField(fldName, inputText, invalidMsg);
            	return;
            }
