...
jslet.ui.DBImage = jslet.Class.create(jslet.ui.DBControl, {
    ...
    initialize: function ($super, el, params) {
		var Z = this;
        if (!Z.allProperties)
            Z.allProperties = 'dataset,field,srcType,locked,baseUrl';
    ...
    bind: function () {
        ...
        if (Z.srcType == undefined || Z.srcType.toLowerCase() == 'file') {
            Z.srcType = 'file';
            Z.checkDataField()
        } else {
            Z.srcType = 'db';
            ...
            if (!Z.baseUrl)
                throw new Error(jslet.formatString(jslet.locale.DBControl.expectedProperty,
						['baseUrl']));
            Z.checkDataset()
        }
        Z.renderAll()
    ...
    refreshControl: function (evt, isForce) {
        ...
        if (evt.eventType == jslet.data.UpdateEvent.SCROLL
            ...
            try {
                var srcURL;
                if (Z.srcType == 'file') {
                    srcURL = Z.dataset.fieldValue(Z.field);
                    if (!srcURL)
                        srcURL = '';
                    else {
                        if (Z.baseUrl)
                    ...
                    srcURL = Z.dataset.fieldValue(Z.dataset
							.keyValue());
                    if (srcURL)
                        srcURL = Z.baseUrl
							+ '?'
