...
jslet.ui.DBText = jslet.Class.create(jslet.ui.DBControl, {
    ...
    initialize: function ($super, el, params) {
        var Z = this;
        if (!Z.allProperties)
            Z.allProperties = 'dataset,field,beforeUpdateToDataset,maxValue,minValue,enableInvalidTip';
    ...
    doKeypress: function (event) {
        ...
        keyCode = event.which;
        if (!fldObj.validator.checkInputChar(String.fromCharCode(keyCode)))
    ...
    refreshControl: function (evt, isForce) {
        ...
        if (evt.eventType == jslet.data.UpdateEvent.METACHANGE) {
            if (evt.eventInfo.enabled != undefined)
                Z.disabled = !evt.eventInfo.enabled;
            ...
            if (document.activeElement != Z.el || Z.el.readOnly)
                value = Z.dataset.getFieldText(Z.field, false);
            else
                value = Z.dataset.getFieldText(Z.field, true);
			if(Z.editMask){
    ...
    updateToDataset: function () {
        ...
        try {
            ...
            Z.dataset.setFieldText(Z.field, value)
        } finally {
...
jslet.ui.DBPassword.htmlTemplate = '<textarea></textarea>';
