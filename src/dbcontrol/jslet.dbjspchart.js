...
jslet.DBJspChart = jslet.Class.create(jslet.DBControl, {
	...
	refreshControl : function(evt) {
		...
		if (evt.eventType == jslet.UpdateEvent.UPDATEALL) {
			...
			var isMultiValueFld = arrValueFields.length > 1;
			var fldName, f;
			var cnt = arrValueFields.length;
			if (isMultiValueFld) {
				for (var i = 0; i < cnt; i++) {
					fldName = arrValueFields[i];
					f = this.dataset.getField(fldName);
					if (!f)
			...
			try {
				...
				var preRecno = this.dataset.recno();
				try {
					var data, categoryValue;
					var recCnt = this.dataset.recordCount();
					for (var k = 0; k < recCnt; k++) {
						...
						if (!isMultiValueFld) {
							...
							for (var i = 0; i < cnt; i++) {
								...
								data[categoryFld] = categoryValue;
								fldName = arrValueFields[i];
								data[titleFld] = this.dataset.getField(fldName)
										.label();
								data[valueFld] = this.dataset
										.fieldValue(fldName);
								arrChartData.push(data)
				...
				new Ajax.Request(this.chartUrl, {
							...
							onSuccess : function(result) {
								...
								if (hostAndPort) {
									var k = s.indexOf('src="');
									var s1 = s.substr(0, k + 5);
									var s2 = s.substr(k + 5);
									s = s1 + hostAndPort + s2
								};
								$(selfchartdiv).update(s)
							},
