...
jslet.ui.htmlclass.TABLECLASS = {
    ...
    celldiv: 'jl-tbl-cell jl-border-box',
    scrollBarWidth: 17,
    selectColWidth: 22,
...
jslet.ui.TableColumn = function () {
    var Z = this;
    Z.hasTotal = false;//Boolean, true - this field value will be summed into total section, only Number field can set this property
	...
	Z.visible = true; //Boolean, specified column visible or not 
...
jslet.ui.AbstractDBTable = jslet.Class.create(jslet.ui.DBControl, {
    ...
    initialize: function ($super, el, params) {
        ...
        Z.allProperties = 'dataset,fixedRows,fixedCols,hasSeqCol,hasSelectCol,readOnly,hideHead,disableHeadSort,onlySpecifiedCol,selectBy,rowHeight,onRowClick,onRowDblClick,onSelect,onSelectAll,onCustomSort,treeField,columns,subgroup,totalFields';
        ...
        Z.headRowHeight = 25;
        /**
         * {String} Display table as tree style. If this property is set, the dataset must be a tree style dataset, 
        ...
        /**
         * {Event} Fired when user click table row.
         *  Pattern: 
         *    function(event}{}
         *    //event: Js mouse event
         */
        Z.onRowClick = null; //event handler
        /**
         * {Event} Fired when user double click table row.
         * Pattern: 
         *    function(event}{}
         *    //event: Js mouse event
         */
        Z.onRowDblClick = null; //event handler
        /**
...
//        Z.charWidth = jslet.ui.textMeasurer.getWidth('M');
	...
	_createFrame: function () {
		...
		var dbtableframe = [
				...
				'<td class="jl-tbl-fixedcol-td"><div class="jl-tbl-fixedcol"><table><tbody /></table><table><tbody /></table><div class="jl-tbl-content-div"><table><tbody /></table></div><table><tbody /></table></div></td>',
				'<td><div class="jl-tbl-contentcol"><div><table class="jl-tbl-content-table"><tbody /></table></div><div><table class="jl-tbl-content-table"><tbody /></table></div><div class="jl-tbl-content-div"><table class="jl-tbl-content-table"><tbody /></table></div><table class="jl-tbl-content-table jl-tbl-fixedcol-td"><tbody /></table></div></td>',
		...
		Z.headSectionHt = Z.maxHeadRows * Z.headRowHeight + 2;//jQuery(Z.rightHeadTbl.parentNode).outerHeight();
	...
	_checkHoriOverflow: function(){
		var Z = this;
		Z._isHoriOverflow = Z.rightHeadTbl.scrollWidth > Z.rightHeadTbl.parentNode.parentNode.clientWidth;	
...
//		var totalWidth = jQuery(Z.rightHeadTbl).width();
	...
	_ajustTableBorder: function () {
		...
		if (hasHead) {
			if (hasLeft)
				setLeftBorder(Z.leftHeadTbl);
		}
		if (hasLeft)
			setLeftBorder(Z.leftContentTbl);
	},
...
//		jqBody.css('cursor','e-resize');
	...
	_doSort: function (sortField, isMultiSort) {
		...
		for (var i = 0, len = Z._sortFields.length; i < len; i++) {
			...
			for (var j = 0; j < fldCnt; j++) {
				...
				lkf = f.lookupField();
				if (lkf != null && !lkf.multiSelect()//if field can hold multiple values, it can't use code field to sort
	...
	_doSelectBoxClick: function (event) {
		...
		try {
			...
			try {
				...
				if (oJslet.onSelect) {
					var flag = oJslet.onSelect(ocheck.checked);
	...
	_renderRow: function (sectionNum) {
		var Z = this;
		...
		var otr, oth, colCfg, isfirstCol, fldCnt = Z.innerColumns.length;
	...
	_fillTotalSection: function () {
		...
		for (var i = 0, len = Z.innerColumns.length; i < len; i++) {
			...
			f = Z.dataset.getField(cobj.field);
			otd.innerHTML =jslet.formatNumber(cobj.totalValue, f
						.displayFormat());
			otd.style.textAlign = f.alignment()
		}
	},
		...
		if (hasLeft) {
			...
			for (var j = 0, clen = cells.length; j < clen; j++) {
				...
				if (otd.isSelectCol) {
					...
					ocheck.checked = Z.dataset.isSelected();
					continue
				}
	...
	expandAll: function () {
		this.listvm.expandAll(function () { this._fillData(); });
	},

	collapseAll: function () {
		this.listvm.collapseAll(function () { Z._fillData(); });
	},
		...
		if (evt.eventType == jslet.data.UpdateEvent.METACHANGE) {
			...
			try {
				for (var i = 0, cnt = leftFixedBody.rows.length; i < cnt; i++) {
					...
					rec = Z.dataset.getRecord();
					checked = rec._selected ? true : false;
					otd = otr.cells[col];
					ocheckbox = otd.firstChild;
					ocheckbox.checked = checked;
					ocheckbox.defaultChecked = checked
...
jslet.ui.EditableCellRender =  jslet.Class.create(jslet.ui.CellRender, {
    createCell: function (otd, colCfg, rowNum) {
        ...
        var f = Z.dataset.getField(fldName);
        otd.style.textAlign = f.alignment();
        otd.innerHTML = '';
...
jslet.ui.SelectCellRender = jslet.Class.create(jslet.ui.CellRender, {
   ...
   createCell: function (otd, colCfg) {
		otd.noWrap = true;
		otd.style.textAlign = 'center';
        var Z = this, jqCheck = jQuery(document.createElement('input'));
		jqCheck.attr('type', 'checkbox').attr('checked',Z.dataset.isSelected()).addClass('jl-tbl-select-check');
		jqCheck.click(Z._doSelectBoxClick);
		otd.appendChild(jqCheck[0]);
    },

    refreshCell: function (otd, colCfg) {
        if (!colCfg || colCfg.noRefresh)
            return;
		var ocheck = otd.firstChild;
		ocheck.checked = this.dataset.isSelected();
    }
});
...
jslet.ui.BoolCellRender = jslet.Class.create(jslet.ui.DefaultCellRender, {
    refreshCell: function (otd, colCfg) {
        ...
        var jqDiv = jQuery(otd.firstChild);
        var Z = this;
        var fldName = colCfg.field, f = Z.dataset.getField(fldName);
        if (f.trueValue == Z.dataset.fieldValue(fldName)) {
...
jslet.ui.TreeCellRender = jslet.Class.create(jslet.ui.CellRender, {
    ...
    refreshCell: function (otd, colCfg) {
        ...
        var arrSpan = odiv.childNodes;
        var Z = this;

        var level = Z.dataset.levelValue();
        ...
        var indentWidth = (!hasChildren ? level + 1 : level) * jslet.ui.TreeCellRender.iconWidth;
        var oindent = arrSpan[0];
        ...
        var fldName = colCfg.field, f = Z.dataset.getField(fldName), text;
        try {
            text = Z.dataset.getFieldText(fldName)
        } catch (e) {
            text = 'error: ' + e.message
        ...
        otd.title = text;
        if (f.urlExpr()) {
            var url = '<a href=' + f.calcUrl();
            var target = f.urlTarget();
            if (target)
                url += ' target=' + target;
            url += '>' + text + '</a>';
            text = url
