...
jslet.ui.DBTreeView = jslet.Class.create(jslet.ui.DBControl, {
    ...
    initialize: function ($super, el, params) {
        var Z = this;
        Z.allProperties = 'dataset,displayFields,hasCheckbox,relativeCheck,codeField,codeFormat,keyField,parentField,iconWidth,checkboxReadOnly,onItemDblClick,iconClassField,onGetIconClass,onCreateContextMenu';
    ...
    bind: function () {
        ...
        Z.relativeCheck = Z.relativeCheck ? true : false;
	...
	_initFrame: function(){
		...
		for(var i = 0, cnt = Z.nodeCount; i < cnt; i++){
			...
			tmpl.push('="3"></td><td class="jl-tree-text" nowrap="nowrap"></td></tr></table>');
		}
	...
	resize: function(){
		...
		if(height != Z.oldHeight){
			Z.oldHeight = height;
			Z.treePanelHeight = jqEl.innerHeight();
			Z.nodeCount = Math.floor(height / Z.nodeHeight);
			Z._initFrame();
			flag = false;//_initFrame will invoke _fillData()
		}
		if(flag)
			Z._fillData();
	},
	...
	_doRowDblClick: function(node){
		...
		if(this.onItemDblClick && !nodeType)
			this.onItemDblClick()
	},
	
	_doCheckBoxClick: function(){
		var Z = this;
		if (Z.checkboxReadOnly)
			return;
		...
		Z.listvm.checkNode(!node.state? 1:0, Z.relativeCheck);
	},
	...
	_fillNode: function(node, rowNo){
        ...
        if (Z.hasCheckbox) {
            ...
            ocheckbox.className = Z._getCheckClassName(Z.dataset.getRecord()._selected);
        ...
        if (Z.onGetIconClass) {
            var iconClsId = null;
            if (Z.onGetIconClass)
                iconClsId = Z.onGetIconClass(Z.dataset.keyValue(), item.level, item.islast); //keyValue, level, isLeaf
	...
	_updateCheckboxState: function(){
			...
			try{
				for(var i = 0, cnt = nodes.length; i < cnt; i++){
					...
					row.cells[cellCnt- 3].className = Z._getCheckClassName(Z.dataset.getRecord()._selected);
				}
			}finally{
				Z.dataset.endSilenceMove(b)
			}
    ...
    _createContextMenu: function () {
        ...
        if (Z.hasCheckbox && !Z.relativeCheck) {
        ...
        Z.el.oncontextmenu = function (event) {
    ...
    _menuItemClick: function (menuid, checked, contextObj) {
        if (menuid == 'expandAll')
            contextObj.expand(null, true);
        else
            if (menuid == 'collapseAll')
                contextObj.collapse(null, true);
            else
                if (menuid == 'checkAll')
                    contextObj.checkNode(null, true,true);
                else
                    if (menuid == 'uncheckAll')
                        contextObj.checkNode(null, false,true);
    },
