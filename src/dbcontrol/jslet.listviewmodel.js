    ...
    this.refreshModel = function () {
        ...
        try {
            ...
            while (!ds.isEof()) {
				...
				state = currRec._selected;
				if(!state)
					state = 0;
					
                node = { parent: pnode, recno: recno, keyvalue: keyValue, expanded: currRec._expanded_, state: state, isbold: 0, level: level };
				pnodes.push(node);
...
//    		return needShowRows ? needShowRows[rowno]: null;
        ...
        var preRowno = currentRowno, recno, currRow=null;
	...
	this.checkNode = function(state, relativeCheck){
		...
		dataset.getRecord()._selected = node.state;
	...
	this._updateChildState = function(node, state){
		...
		try{
			for(var i = 0, cnt = node.children.length; i < cnt; i++){
				...
				dataset.getRecord()._selected = state;
	...
	this._updateParentState = function(node, state){
		...
		if(pNode.state != newState){
			...
			try{
				...
				dataset.getRecord()._selected = newState;
			}finally{
				dataset.endSilenceMove(b);
			}
		}
